...
adata_vis = sc.read_h5ad("ncem/map_luad/sp.h5ad")
mod = cell2location.models.Cell2location.load("ncem/map_luad", adata_vis)
...
pickle.dump(adata_vis2, open('ncem/luad_ncem_raw.pkl', 'wb'))




### subset WIKIpathway gene
adata_vis = pickle.load(open('ncem/luad_ncem_raw.pkl', 'rb'))
# Get MSigDB resource
...
## select high sample: P10_T1, P24_T2, P24_T1, P16_T1, P16_T2 and low sample: P10_T4, P15_T1, P15_T2, P25_T1, P25_T2
adata_vis = subset_slide(adata_vis, samples=["P10_T1", "P24_T2", "P24_T1", "P16_T1", "P16_T2", "P10_T4", "P15_T1", "P15_T2", "P25_T1", "P25_T2"])
pickle.dump(adata_vis, open('ncem/luad_ncem_raw_sub.pkl', 'wb'))
...
adata_ncem.obs['sample'] = new_sample
# save NCEM data
adata_ncem.write("ncem/luad_ncem.h5ad")
...
high = ["P10_T1", "P24_T2", "P24_T1", "P16_T1", "P16_T2"]
low = ["P10_T4", "P15_T1", "P15_T2", "P25_T1", "P25_T2"]

high = ["P10_T1", "P24_T2", "P24_T1"]
low = ["P10_T4", "P25_T2"]

adata = sc.read_h5ad("ncem/luad_ncem.h5ad")
adata_high = adata[adata.obs['sample'].isin(high)]
adata_high.write("ncem/luad_ncem_high.h5ad")
adata_low = adata[adata.obs['sample'].isin(low)]
adata_low.write("ncem/luad_ncem_low.h5ad")
...
run_ncem_couple(group='high', cancertype='luad')
run_ncem_couple(group='low', cancertype='luad')


### luad high score group
### Set sender and receiver cell types
ncem_ip = pickle.load(open('ncem/luad_ncem_ip_high.pkl', 'rb'))
# the gene name in ncem_ip is ensemble, change to symbol
adata = sc.read_h5ad("ncem/map_luad/sp.h5ad")
...
effect_df.to_csv("ncem/luad_send_high_effect.csv", index=True)
...
gene_subset = gene_subset[abs(gene_subset['fold change']) > 1].index

### Plot sender and receiver effect analysis results
ncem_ip.sender_effect(receiver=f'{receiver}', gene_subset=list(gene_subset), figsize=(12,12), save = "ncem/sender_receiver", suffix= "luad_high_sender.pdf")
ncem_ip.receiver_effect(sender=f'{sender}', gene_subset=list(gene_subset), figsize=(12,12), save = "ncem/sender_receiver", suffix= "luad_high_receiver.pdf")

### Sender similarity analysis for CD4 T cells
ncem_ip.sender_similarity_analysis(receiver='T cell CD4', figsize=(12,12), save = "ncem/sender_receiver", suffix= "T cell CD4_luad_high_similarity.pdf")





### luad low score group
### Set sender and receiver cell types
ncem_ip = pickle.load(open('ncem/luad_ncem_ip_low.pkl', 'rb'))
# the gene name in ncem_ip is ensemble, change to symbol
adata = sc.read_h5ad("ncem/map_luad/sp.h5ad")
adata_sub = adata[:, ncem_ip.node_feature_names]
ncem_ip.data.celldata.var.index = adata_sub.var['Symbol'].tolist()
...
effect_df = ncem_ip.sender_receiver_values(sender=f'{sender}', receiver=f'{receiver}')
#Write results
effect_df.to_csv("ncem/luad_send_low_effect.csv", index=True)
#Select statistically and biologically meaningful genes
gene_subset = effect_df[effect_df['qvalue'] < 0.05]
gene_subset = gene_subset[~gene_subset.index.str.startswith(('RPL', 'RPS'))]
gene_subset = gene_subset[abs(gene_subset['fold change']) > 1].index
...
### Plot sender and receiver effect analysis results
ncem_ip.sender_effect(receiver=f'{receiver}', gene_subset=list(gene_subset), figsize=(12,12), save = "ncem/sender_receiver", suffix= "luad_low_sender.pdf")
ncem_ip.receiver_effect(sender=f'{sender}', gene_subset=list(gene_subset), figsize=(12,12), save = "ncem/sender_receiver", suffix= "luad_low_receiver.pdf")

### Sender similarity analysis for CD4 T cells
ncem_ip.sender_similarity_analysis(receiver='T cell CD4', figsize=(12,12), save = "ncem/sender_receiver", suffix= "T cell CD4_luad_low_similarity.pdf")
