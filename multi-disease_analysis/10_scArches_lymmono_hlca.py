...
lymmono_ref_hlca = ref_hlca_sub[~ref_hlca_sub.obs_names.isin(lymmono.obs.cell_ID), :]
...
lymmono_ref_hlca.layers["counts"] = lymmono_ref_hlca.X.copy()
...
sca.models.SCVI.setup_anndata(lymmono_ref_hlca, batch_key=condition_key)
vae = sca.models.SCVI(lymmono_ref_hlca)
vae.train(max_epochs=vae_epochs)

reference_latent = sc.AnnData(vae.get_latent_representation())
reference_latent.obs["cell_type"] = lymmono_ref_hlca.obs[cell_type_key].tolist()
reference_latent.obs["dataset"] = lymmono_ref_hlca.obs[condition_key].tolist()

sc.pp.neighbors(reference_latent, n_neighbors=8)
sc.tl.leiden(reference_latent)
sc.tl.umap(reference_latent)
vae.save('scarches/ref_model/lymmono_ref_hlca', overwrite=True)
...
# the gene of target data and query data need to be the same
# contact the adata for same gene and the new adata for missing gene
genes_to_extract = lymmono_ref_hlca.var_names
...
missing_genes = lymmono_ref_hlca.var_names.difference(common_genes)
...
# gene sort by reference dataset (lymmono_ref_hlca)
adata_to_map_augmented = adata_to_map_augmented[:, lymmono_ref_hlca.var.index.tolist()].copy()
(adata_to_map_augmented.var.index == lymmono_ref_hlca.var.index).all()
adata_to_map_augmented.obs['study'] = adata_to_map.obs['donor_status']

# train model for target data
model = sca.models.SCVI.load_query_data(
    adata_to_map_augmented,
    'scarches/ref_model/lymmono_ref_hlca',
    freeze_dropout = True,
)
...
model.save('scarches/query_model/lymmono_ref_hlca', overwrite=True)


## Get latent representation of reference + query dataset and compute UMAP
lymmono_ref_hlca.obs['ref_query'] = 'refdata'
adata_to_map_augmented.obs['ref_query'] = 'querydata'
adata_to_map_augmented.var.columns=['GeneID']
adata_full = lymmono_ref_hlca.concatenate(adata_to_map_augmented)
...
plt.savefig('scarches/lymmono_hlca.pdf', dpi=300, format='pdf', bbox_inches='tight')
full_latent.write_h5ad('scarches/lymmono_hlca_full.h5ad')
