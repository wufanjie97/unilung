...
tbcell_ref_luca = ref_luca_sub[~ref_luca_sub.obs_names.isin(tbcell.obs.cell_ID), :]
...
tbcell_ref_luca.layers["counts"] = tbcell_ref_luca.X.copy()
...
sca.models.SCVI.setup_anndata(tbcell_ref_luca, batch_key=condition_key)
vae = sca.models.SCVI(tbcell_ref_luca)
vae.train(max_epochs=vae_epochs)

reference_latent = sc.AnnData(vae.get_latent_representation())
reference_latent.obs["cell_type"] = tbcell_ref_luca.obs[cell_type_key].tolist()
reference_latent.obs["dataset"] = tbcell_ref_luca.obs[condition_key].tolist()

sc.pp.neighbors(reference_latent, n_neighbors=8)
sc.tl.leiden(reference_latent)
sc.tl.umap(reference_latent)
vae.save('scarches/ref_model/tbcell_ref_luca', overwrite=True)
...
genes_to_extract = tbcell_ref_luca.var_names
...
missing_genes = tbcell_ref_luca.var_names.difference(common_genes)
...
# gene sort by reference dataset (tbcell_ref_luca)
adata_to_map_augmented = adata_to_map_augmented[:, tbcell_ref_luca.var.index.tolist()].copy()
(adata_to_map_augmented.var.index == tbcell_ref_luca.var.index).all()
adata_to_map_augmented.obs['dataset'] = adata_to_map.obs['donor_status']

# train model for target data
model = sca.models.SCVI.load_query_data(
    adata_to_map_augmented,
    'scarches/ref_model/tbcell_ref_luca',
    freeze_dropout = True,
)
...
model.save('scarches/query_model/tbcell_ref_luca', overwrite=True)


## Get latent representation of reference + query dataset and compute UMAP
tbcell_ref_luca.obs['ref_query'] = 'refdata'
adata_to_map_augmented.obs['ref_query'] = 'querydata'
adata_to_map_augmented.var.columns=['GeneID']
adata_full = tbcell_ref_luca.concatenate(adata_to_map_augmented)
...
plt.savefig('scarches/tbcell_luca.pdf', dpi=300, format='pdf', bbox_inches='tight')
full_latent.write_h5ad('scarches/tbcell_luca_full.h5ad')