...
ref_pant_sub =  sc.read_h5ad('scarches/ref_pant_sub_latest.h5ad')
tbcell_ref_pant = ref_pant_sub[~ref_pant_sub.obs_names.isin(tbcell.obs.cell_ID), :]


## train reference model for ref_pant
condition_key = 'cancerType'
cell_type_key = 'meta.cluster'
vae_epochs = 500
surgery_epochs = 500

tbcell_ref_pant.layers["counts"] = tbcell_ref_pant.X.copy()
...
sca.models.SCVI.setup_anndata(tbcell_ref_pant, batch_key=condition_key)
vae = sca.models.SCVI(tbcell_ref_pant)
vae.train(max_epochs=vae_epochs)

reference_latent = sc.AnnData(vae.get_latent_representation())
reference_latent.obs["cell_type"] = tbcell_ref_pant.obs[cell_type_key].tolist()
reference_latent.obs["cancer"] = tbcell_ref_pant.obs[condition_key].tolist()

sc.pp.neighbors(reference_latent, n_neighbors=8)
sc.tl.leiden(reference_latent)
sc.tl.umap(reference_latent)
vae.save('scarches/ref_model/tbcell_ref_pant', overwrite=True)
...
genes_to_extract = tbcell_ref_pant.var_names
...
missing_genes = tbcell_ref_pant.var_names.difference(common_genes)
...
# gene sort by reference dataset (tbcell_ref_pant)
adata_to_map_augmented = adata_to_map_augmented[:, tbcell_ref_pant.var.index.tolist()].copy()
(adata_to_map_augmented.var.index == tbcell_ref_pant.var.index).all()
adata_to_map_augmented.obs['cancerType'] = adata_to_map.obs['donor_status']

# train model for target data
model = sca.models.SCVI.load_query_data(
    adata_to_map_augmented,
    'scarches/ref_model/tbcell_ref_pant',
    freeze_dropout = True,
)
...
model.save('scarches/query_model/tbcell_ref_pant', overwrite=True)


## Get latent representation of reference + query dataset and compute UMAP
tbcell_ref_pant.obs['ref_query'] = 'refdata'
adata_to_map_augmented.obs['ref_query'] = 'querydata'
adata_to_map_augmented.var.columns=['GeneSymbol']
adata_full = tbcell_ref_pant.concatenate(adata_to_map_augmented)
...
full_latent.obs['cancer'] = adata_full.obs['cancerType'].tolist()
...
plt.savefig('scarches/tbcell_pant.pdf', dpi=300, format='pdf', bbox_inches='tight')
full_latent.write_h5ad('scarches/tbcell_pant_full.h5ad')