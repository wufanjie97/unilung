...
meta <- sce@meta.data
meta$meta.cluster <- meta$donor_status

meta$loc <- meta$mono_anno
#meta$loc <- meta$bcell_anno

OR.list <- do.tissueDist(cellInfo.tb = meta,
                         out.prefix = paste0(celltype,"_OR_plot"),
                         pdf.width = 5,
                         pdf.height = 8,
                         verbose=1
)
...
# save OR value
a <- OR.list[["OR.dist.tb"]] %>% as.data.frame()
rownames(a) <- a$rid
a <- a[,-1]
a <- na.omit(a)
# save P value
b <- OR.list$count.dist.melt.ext.tb[,c(1,2,6)]
b <- spread(b,key = "cid", value = "adj.p.value")
b <- data.frame(b[,-1],row.names = b$rid)
b <- b[rownames(a),]

col <- viridis(11,option = "D")
c = ifelse(b >= 0.05&(a>2|a<1), "",
           ifelse(b<0.0001&(a>2|a<1),"****",
                  ifelse(b<0.001&(a>2|a<1),"***",
                         ifelse(b<0.01&(a>2|a<1),"**",
                                ifelse(b < 0.05&(a>2|a<1),"*","")))))

bk=c(seq(0,1.49,by=0.01),seq(1.5,2.5,by=0.01))
pdf(paste0(celltype,'_group_OR.pdf'),width = 6,height = 10)
pheatmap(a[,], 
         border_color = "NA", 
         fontsize = 9,
         cellheight = 12,
